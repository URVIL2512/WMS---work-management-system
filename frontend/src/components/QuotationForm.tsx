...
export default function QuotationForm({ onClose, onSuccess, quotationToEdit }: QuotationFormProps) {
  ...
  // Financial fields (only editable in quotation)
  ...
  const loadQuotationData = () => {
    ...
    setTcsPercent(quotationToEdit.tcsPercent || 0);
    setRemittanceCharges(quotationToEdit.remittanceCharges || 0);
    setPackagingCost(quotationToEdit.packagingCost || 0);
    setTransportCost(quotationToEdit.transportCost || 0);
    if (quotationToEdit.tdsPercent > 0) {
      setTaxMode('TDS');
    } else if (quotationToEdit.tcsPercent > 0) {
      setTaxMode('TCS');
  ...
  // NEW CALCULATION LOGIC (Quotation Only)
  // itemTotal = sum(item.rate * item.quantity)
  const itemTotal = items.reduce((sum, item) => {
    return sum + ((item.rate || 0) * (item.quantity || 1));
  }, 0);

  // processTotal = sum(each item.process.processCost * process.quantity)
  const processTotal = items.reduce((sum, item) => {
    const itemProcessTotal = (item.processes || []).reduce((processSum, process) => {
      return processSum + ((process.processCost || 0) * (process.quantity || 1));
    }, 0);
    return sum + itemProcessTotal;
  }, 0);

  // subtotal = itemTotal + processTotal
  const subtotal = itemTotal + processTotal;

  // gst = subtotal * 0.18 (using gstPercent from state)
  const gst = Math.round((subtotal * (gstPercent / 100)) * 100) / 100;

  // finalTotal = subtotal + gst + packagingCost + transportCost
  const finalTotal = subtotal + gst + (packagingCost || 0) + (transportCost || 0);

  // Legacy calculations (for backward compatibility with existing UI)
  const baseAmount = items.reduce((sum, item) => sum + (item.itemTotal || item.amount), 0);
  ...
  const calculateTDS = (): number => {
    ...
    return Math.round(((subtotal * tdsPercent) / 100) * 100) / 100;
  };
  ...
  const calculateTCS = (): number => {
    ...
    return Math.round(((subtotal * tcsPercent) / 100) * 100) / 100;
  };

  const tds = calculateTDS();
  const tcs = calculateTCS();
  
  // Legacy total calculation (includes TDS/TCS/Remittance for display)
  const quotationTotal = finalTotal + tcs - tds + remittanceCharges;
  const invoiceTotal = quotationTotal;
  
  // Currency Conversion (Receivable Amount = Quotation Total × Exchange Rate)
  const receivableAmount = quotationTotal * (exchangeRate || 1);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const quotationData = {
        ...
        // Save calculated snapshot (no backend recalculation)
        itemTotal: itemTotal,
        ...
        tcsAmount: tcs,
        remittanceCharges: remittanceCharges,
        totalSubTotal: subtotal, // Legacy field
        totalTaxAmount: gst, // Legacy field
        invoiceTotal: invoiceTotal,
        receivableAmount: receivableAmount,
        totalAmount: finalTotal, // Use finalTotal as totalAmount
        paymentTerms: '',
        remarks: customerNotes,
        currency: currency,
        exchangeRate: exchangeRate,
      ...
      <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="space-y-3 max-w-full">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Packaging Cost (₹)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Transport Cost (₹)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
              ...
              <div className="flex justify-end mt-4">
                <div className="w-full md:w-[35%] bg-gray-50 rounded-lg p-4 shadow-md border-2 border-gray-200">
                  ...
                  {/* Product Amount (itemTotal) */}
                  <div className="flex justify-between items-center py-2 border-b border-gray-300">
                    <span className="font-medium text-gray-700">Product Amount:</span>
                    <span className="font-semibold text-gray-900 transition-all duration-300">{currency} {itemTotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>

                  {/* Process Charges (processTotal) */}
                  {processTotal > 0 && (
                    <div className="flex justify-between items-center py-2 border-b border-gray-300">
                      <span className="font-medium text-gray-700">Process Charges:</span>
                      <span className="font-semibold text-gray-900 transition-all duration-300">{currency} {processTotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  )}

                  {/* Subtotal */}
                  <div className="flex justify-between items-center py-2 border-b border-gray-300 bg-gray-100 px-2 rounded">
                    <span className="font-medium text-gray-700">Subtotal:</span>
                    <span className="font-semibold text-gray-900 transition-all duration-300">{currency} {subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                  ...
                  {(packagingCost || 0) > 0 && (
                    <div className="flex justify-between items-center py-2 border-b border-gray-300">
                      ...
                      <span className="font-semibold text-gray-900 transition-all duration-300">{currency} {(packagingCost || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  )}
                  ...
                  {(transportCost || 0) > 0 && (
                    <div className="flex justify-between items-center py-2 border-b border-gray-300">
                      ...
                      <span className="font-semibold text-gray-900 transition-all duration-300">{currency} {(transportCost || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-center py-2 border-t-2 border-gray-400 pt-3 mt-2">
                    ...
                    <span className="font-bold text-lg text-gray-900 transition-all duration-300">{currency} {finalTotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>

                  {/* TDS */}
                  {taxMode === 'TDS' && tds > 0 && (
                    <div className="flex justify-between items-center py-2 border-b border-gray-300 bg-red-50 px-2 rounded mt-2">
                      <span className="font-medium text-gray-700">TDS ({tdsPercent}%):</span>
                      <span className="font-semibold text-red-700 transition-all duration-300">- {currency} {tds.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  )}
                  ...
                  {/* TCS */}
                  {taxMode === 'TCS' && tcs > 0 && (
                    <div className="flex justify-between items-center py-2 border-b border-gray-300 bg-blue-50 px-2 rounded mt-2">
                      <span className="font-medium text-gray-700">TCS ({tcsPercent}%):</span>
                      <span className="font-semibold text-blue-700 transition-all duration-300">{currency} {tcs.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  )}
                  ...
                  {/* Remittance Charges */}
                  {remittanceCharges > 0 && (
                    <div className="flex justify-between items-center py-2 border-b border-gray-300 bg-orange-50 px-2 rounded mt-2">
                      <span className="font-medium text-gray-700">Remittance Charges:</span>
                      <span className="font-semibold text-orange-700 transition-all duration-300">+ {currency} {remittanceCharges.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  )}

                  {/* Quotation Total (with TDS/TCS/Remittance) */}
                  <div className="flex justify-between items-center py-2 border-t-2 border-gray-400 pt-3 mt-2">
                    <span className="font-semibold text-gray-700">Quotation Total:</span>
                    <span className="font-bold text-lg text-gray-900 transition-all duration-300">{currency} {invoiceTotal.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
