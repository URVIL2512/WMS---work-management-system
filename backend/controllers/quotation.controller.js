  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      try {
        ...
        // Use snapshot values from quotation (no recalculation)
        const subtotal = quotation.subtotal || quotation.totalSubTotal || 0;
        const gst = quotation.gst || quotation.totalTaxAmount || 0;
        const finalTotal = quotation.finalTotal || quotation.totalAmount || 0;
        
        doc.text(`Subtotal: ₹${subtotal.toLocaleString('en-IN')}`, 400, currentY, { align: 'right', width: 150 });
        currentY += 12;
        
        if (quotation.gstPercent && quotation.gstPercent > 0 && gst > 0) {
          doc.text(`GST (${quotation.gstPercent}%): ₹${gst.toLocaleString('en-IN')}`, 400, currentY, { align: 'right', width: 150 });
          currentY += 12;
        }
        ...
        doc.fontSize(12);
        doc.font('Helvetica-Bold');
        doc.text(`Total Amount: ₹${finalTotal.toLocaleString('en-IN')}`, 400, currentY, { align: 'right', width: 150 });
        currentY += 25;
...
export const createQuotation = async (req, res, next) => {
  try {
    ...
    // Use provided snapshot values (calculated in frontend) or calculate if not provided (backward compatibility)
    const itemTotal = req.body.itemTotal !== undefined ? req.body.itemTotal : 0;
    const processTotal = req.body.processTotal !== undefined ? req.body.processTotal : 0;
    const subtotal = req.body.subtotal !== undefined ? req.body.subtotal : (itemTotal + processTotal);
    const gst = req.body.gst !== undefined ? req.body.gst : (subtotal * (gstPercent || 0) / 100);
    const finalTotal = req.body.finalTotal !== undefined ? req.body.finalTotal : (subtotal + gst + (packagingCost || 0) + (transportCost || 0));
    const priceLocked = req.body.priceLocked !== undefined ? req.body.priceLocked : true;
    
    // Use finalTotal as totalAmount (backward compatibility)
    const totalAmount = req.body.totalAmount !== undefined ? req.body.totalAmount : finalTotal;
    ...
    const quotation = await Quotation.create({
      ...
      // Save calculated snapshot (no recalculation)
      itemTotal,
      processTotal,
      subtotal,
      gst,
      ...
      totalSubTotal: req.body.totalSubTotal !== undefined ? req.body.totalSubTotal : subtotal,
      totalTaxAmount: req.body.totalTaxAmount !== undefined ? req.body.totalTaxAmount : gst,
      createdBy: req.user._id,
      status
    });
...
export const updateQuotation = async (req, res, next) => {
  ...
  try {
    ...
    // Use provided snapshot values (calculated in frontend) or keep existing values
    const finalGstPercent = gstPercent !== undefined ? gstPercent : quotation.gstPercent || 0;
    const finalPackagingCost = packagingCost !== undefined ? (packagingCost || 0) : (quotation.packagingCost || 0);
    const finalTransportCost = transportCost !== undefined ? (transportCost || 0) : (quotation.transportCost || 0);
    
    // Use snapshot values from request body if provided, otherwise keep existing
    const itemTotal = req.body.itemTotal !== undefined ? req.body.itemTotal : quotation.itemTotal || 0;
    const processTotal = req.body.processTotal !== undefined ? req.body.processTotal : quotation.processTotal || 0;
    const subtotal = req.body.subtotal !== undefined ? req.body.subtotal : quotation.subtotal || (itemTotal + processTotal);
    const gst = req.body.gst !== undefined ? req.body.gst : quotation.gst || (subtotal * finalGstPercent / 100);
    const finalTotal = req.body.finalTotal !== undefined ? req.body.finalTotal : quotation.finalTotal || (subtotal + gst + finalPackagingCost + finalTransportCost);
    const priceLocked = req.body.priceLocked !== undefined ? req.body.priceLocked : (quotation.priceLocked !== undefined ? quotation.priceLocked : true);
    
    // Use finalTotal as totalAmount
    const totalAmount = req.body.totalAmount !== undefined ? req.body.totalAmount : finalTotal;
    
    console.log('📝 [UPDATE] Saving snapshot values:', {
      itemTotal,
      processTotal,
      subtotal,
      gst,
